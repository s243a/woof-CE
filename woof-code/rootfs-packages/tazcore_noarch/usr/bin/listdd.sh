...
<window title=\"ListDD: $(gettext 'List Dynamic Dependencies')\" icon-name=\"listdd\" resizable=\"true\">
 <vbox>
  ...
  <entry tooltip-text=\" $(gettext 'Select a binary executable or library file') \" fs-action=\"newfile\" fs-folder=\"/usr\" fs-title=\" $(gettext 'ListDD File Selector') \" secondary-icon-stock=\"gtk-clear\" secondary-icon-tooltip-text=\" $(gettext 'Clear entry') \">
  ...
  <button relief=\"1\" tooltip-text=\" $(gettext 'File selector') \">	
    <input file stock=\"gtk-directory\"></input>
    <action>fileselect:FILEPATH</action>
  </button>
  <button relief=\"1\" tooltip-text=\" $(gettext 'Help') \">	
  ...
  <hbox>
   <button has-focus=\"true\" height-request=\"35\" width-request=\"80\" tooltip-text=\" $(gettext 'Summary report') \" use-underline=\"true\">
    <label>$(gettext '_Summary')</label>
    <action>. listdd.sh -summary</action>
    <action>refresh:INFO</action>
   </button>
   <button height-request=\"35\" width-request=\"80\" tooltip-text=\" $(gettext 'Verbose report') \" use-underline=\"true\">
    <label>$(gettext '_Complete')</label>
    <action>. listdd.sh -complete</action>
    <action>refresh:INFO</action>
   </button>
   <button height-request=\"35\" width-request=\"80\" tooltip-text=\" $(gettext 'Unused direct dependencies') \" use-underline=\"true\">
    <label>$(gettext '_Unused')</label>
    <action>. listdd.sh -unused</action>
    <action>refresh:INFO</action>
   </button>
   <button height-request=\"35\" width-request=\"80\" tooltip-text=\" $(gettext 'Absent / Missing dependencies') \" use-underline=\"true\">
    <label>$(gettext '_Missing')</label>
    <action>. listdd.sh -missing</action>
    <action>refresh:INFO</action>
   </button>
   $CHECK_DEPS
   <button height-request=\"35\" width-request=\"80\" use-underline=\"true\">
...
$GTKDIALOG --geometry 600x400 -p LISTDD_DIALOG > /dev/null 2>&1 </dev/null
unset LISTDD_DIALOG
