...
mkdir -p /tmp/petget-proc/petget #101225
 ...
 *.pet)
  ...
  dd if="${DLPKG_BASE}" of=/tmp/petget-proc/petget/petmd5sum bs=1 skip=${ORIGSIZE} 2>/dev/null
  sync
  MD5SUM="`cat /tmp/petget-proc/petget/petmd5sum`"
  pet2tgz $DLPKG_BASE
  [ $? -ne 0 ] && FLAG="bad"
