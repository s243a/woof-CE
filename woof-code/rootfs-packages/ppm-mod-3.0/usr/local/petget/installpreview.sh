...
#/tmp/petget-proc/petget/current-repo-triad has the repository that installing from.
#100821 bug in Lucid 5.1, /tmp/petget-proc/pup_event_sizefreem had two identical lines.
...
#120904 "examine dependencies" button did not create any /tmp/petget-proc/petget_missing_dbentries-*, workaround.
...
DB_FILE=Packages-`cat /tmp/petget-proc/petget/current-repo-triad` #ex: Packages-slackware-12.2-official
tPATTERN='^'"$TREE1"'|'

#120827 'examine dependencies' button does not work if pkg already installed...
EXAMDEPSFLAG='yes'
ttPTN='^'"$TREE1"'|.*ALREADY INSTALLED'
if [ "`grep "$ttPTN" /tmp/petget-proc/petget/filterpkgs.results.post`" != "" ];then #created by postfilterpkgs.sh
 EXAMDEPSFLAG='no'

fi

#120504 if findnames.sh searched multiple repos, /tmp/petget-proc/petget/current-repo-triad (set in pkg_chooser.sh) might be wrong...
[ -f /tmp/petget-proc/petget/current-repo-triad.previous ] && rm -f /tmp/petget-proc/petget/current-repo-triad.previous
if [ -f /tmp/petget-proc/petget/filterpkgs.results.post ];then
  ALTSEARCHREPO="$(grep "$tPATTERN" /tmp/petget-proc/petget/filterpkgs.results.post | grep '|\[' | cut -f 2 -d '[' | cut -f 1 -d ']')"
 ...
 if [ "$ALTSEARCHREPO" ];then
  mv -f /tmp/petget-proc/petget/current-repo-triad /tmp/petget-proc/petget/current-repo-triad.previous #need to restore old one before exit this script.
  echo -n "$ALTSEARCHREPO" > /tmp/petget-proc/petget/current-repo-triad
 fi
fi
...
SIZEFREEM=`cat /tmp/petget-proc/pup_event_sizefreem | head -n 1` #100821 bug in Lucid 5.1, file had two identical lines.
...
if [ "$DB_dependencies" = "" ];then
 ...
 #/tmp/petget-proc/petget_missingpkgs_patterns has a list of missing dependencies, format ex:
 #|kdebase|
 #|kdelibs|
 #|mesa|
 #|qt|
 ...
 if [ "$MISSINGDEPS_PATTERNS" = "" ];then
  ...
  if [ "$EXAMDEPSFLAG" != "no" ];then #120828
   ...
   <action>echo \"${TREE1}\" > /tmp/petget-proc/petget_installpreview_pkgname</action>
...
if [ ! -f /tmp/petget-proc/install_quietly ]; then
export PREVIEW_DIALOG="<window title=\"$(gettext 'Puppy Package Manager: preinstall')\" icon-name=\"gtk-about\" default_width=\"500\" default_height=\"400\" >
<vbox>
 ...
 <hbox space-expand=\"false\" space-fill=\"false\">
  ${DEPBUTTON}
  <button>
   <label>$(gettext 'Install')${ONLYMSG}</label>
   <action>echo \"${TREE1}\" > /tmp/petget-proc/petget_installpreview_pkgname</action>
...
if [ "$EXIT" != "BUTTON_INSTALL" -a "$EXIT" != "BUTTON_EXAMINE_DEPS" -a "$EXIT" != "BUTTON_PKGS_DOWNLOADONLY" ];then
 [ -f /tmp/petget-proc/petget/current-repo-triad.previous ] && mv -f /tmp/petget-proc/petget/current-repo-triad.previous /tmp/petget-proc/petget/current-repo-triad
 exit
fi
...
if [ "$EXIT" = "BUTTON_EXAMINE_DEPS" ];then
 ...
 #returns with /tmp/petget-proc/petget_missing_dbentries-* has the database entries of missing deps.
 ...
 if [ "$FNDMISSINGDBENTRYFILE" = "" -a ! -f /tmp/petget-proc/install_quietly ];then
...
$(gettext "Please cancel installation, close the Puppy Package Manager, then click the \Zbinstall\ZB icon on the desktop and install the devx SFS file first.")\" 0 0" > /tmp/petget-proc/petget_devx_popup.sh #'geany
   chmod 755 /tmp/petget-proc/petget_devx_popup.sh
   /tmp/petget-proc/petget_devx_popup.sh &
  fi
 fi
 ...
 MAIN_CHK="<checkbox><default>true</default><label>${MAINPKG_NAME} SIZE: ${MAINPKG_SIZE}B</label><variable>CHECK_PKG_${MAIN_REPO}_${MAINPKG_NAME}</variable></checkbox>"
 INSTALLEDSIZEK=0
 ...
 do
  ...
  FRAME_CNT=`cat /tmp/petget-proc/petget_frame_cnt`
  if [ "$ONEREPO_PREV" != "" ];then #next repo, so start a new tab.
   DEP_CNT=0
   FRAME_CNT=`expr $FRAME_CNT + 1`
   echo "$FRAME_CNT" > /tmp/petget-proc/petget_frame_cnt
   #w017 bugfix, prevent double frame closure...
   [ "`cat /tmp/petget-proc/petget_moreframes | tail -n 1 | grep '</frame></vbox>$'`" = "" ] && echo "</frame></vbox>" >> /tmp/petget-proc/petget_moreframes
  fi
  cat $ONEDEPSLIST |
  while read ONELIST
  do
   ...
   DP_CNT=`expr $FRAME_CNT + 1`
   case $DEP_CNT in
    1)
     echo -n "<vbox margin=\"8\"><frame REPOSITORY: ${ONEREPO}>" >> /tmp/petget-proc/petget_moreframes
     echo -n "Deps $DP_CNT|" >> /tmp/petget-proc/petget_tabs
     echo -n "<checkbox><default>true</default><label>${DEP_NAME} SIZE: ${DEP_SIZE}B</label><variable>CHECK_PKG_${ONEREPO}_${DEP_NAME}</variable></checkbox>" >> /tmp/petget-proc/petget_moreframes
    ;;
    8)
     FRAME_CNT=`cat /tmp/petget-proc/petget_frame_cnt`
     FRAME_CNT=`expr $FRAME_CNT + 1`
     if [ $FRAME_CNT -gt 10 ];then #120907
      echo -n "<text use-markup=\"true\"><label>\"<b>$(gettext 'SORRY! Too many dependencies, list truncated. Suggest click Cancel button and install some deps first.')</b>\"</label></text>" >> /tmp/petget-proc/petget_moreframes #120907
     else
      echo -n "<checkbox><default>true</default><label>${DEP_NAME} SIZE: ${DEP_SIZE}B</label><variable>CHECK_PKG_${ONEREPO}_${DEP_NAME}</variable></checkbox>" >> /tmp/petget-proc/petget_moreframes
     fi
     echo "</frame></vbox>" >> /tmp/petget-proc/petget_moreframes
     DEP_CNT=0
     echo "$FRAME_CNT" > /tmp/petget-proc/petget_frame_cnt
    ;;
    *)
   ...
   echo "$INSTALLEDSIZEK" > /tmp/petget-proc/petget_installedsizek
  done
  INSTALLEDSIZEK=`cat /tmp/petget-proc/petget_installedsizek`
  FRAME_CNT=`cat /tmp/petget-proc/petget_frame_cnt`
  [ $FRAME_CNT -gt 10 ] && break #too wide! 120907
 done
 TABS="`cat /tmp/petget-proc/petget_tabs`"
 MOREFRAMES="`cat /tmp/petget-proc/petget_moreframes`"
 #make sure last frame has closed...
 [ "`echo "$MOREFRAMES" | tail -n 1 | grep '</frame></vbox>$'`" = "" ] && MOREFRAMES="${MOREFRAMES}</frame></vbox>"
...
if [ ! -f /tmp/petget-proc/install_quietly ]; then
 export DEPS_DIALOG="<window title=\"$(gettext 'Puppy Package Manager: dependencies')\" icon-name=\"gtk-about\" default_width=\"600\" default_height=\"400\" width=\"600\" height=\"400\">
<vbox>
...
<vbox scrollable=\"true\">
 ...
 ${MOREFRAMES}
	...
	<vbox> 
	 <text use-markup=\"true\"><label>$(gettext "Sometimes Puppy's automatic dependency checking comes up with a list that may include packages that don't really need to be installed, or are already installed under a different name. If uncertain, just accept them all, but if you spot one that does not need to be installed, then un-tick it.")</label></text>
	 <text use-markup=\"true\"><label>\" \"</label></text>
	 <text use-markup=\"true\"><label>$(gettext 'Puppy usually avoids listing the same package more than once if it exists in two or more repositories. However, if the same package is listed twice, choose the one that seems to be most appropriate.')</label></text>
	  ...
	  <vbox>
	   <text><label>$(gettext 'Click to see the hierarchy of the dependencies:')</label></text>
		<button>
		 ...
		 <action>/usr/local/bin/defaulttextviewer /tmp/petget-proc/petget_deps_visualtreelog & </action>
		</button>
	  </vbox>
  ...
  <vbox>
  <frame>
  <text use-markup=\"true\"><label>\"$(gettext 'If all of the above packages are selected, the total installed size will be') ${INSTALLEDSIZEK}KB (${INSTALLEDSIZEM}MB). $(gettext 'The free space available for installation is') ${SIZEFREEK}KB (${SIZEFREEM}MB). ${MSGWARN2}\"</label></text>
  </frame>
  </vbox>
...
EXIT=$xEXIT"
  [ "$DEPS_TOINSTALL" != "" ] && echo "$DEPS_TOINSTALL"  | cut -f 1 -d '=' \
   | cut -f 4-10 -d '_'  >> /tmp/petget-proc/pkgs_to_install_done
  rm -f /tmp/petget-proc/petget_moreframes
fi
 ...
 if [ "`echo "$RETPARAMS" | grep '^EXIT' | grep -E 'BUTTON_PKGS_INSTALL|BUTTON_PKGS_DOWNLOADONLY'`" != "" ];then
  ...
  do
   ...
   opPATTERN='^'"$ONEPKG"'|'
   grep -v "$opPATTERN" /tmp/petget-proc/petget_missing_dbentries-Packages-${ONEREPO} > /tmp/petget-proc/petget_tmp
   mv -f /tmp/petget-proc/petget_tmp /tmp/petget-proc/petget_missing_dbentries-Packages-${ONEREPO}
  done
 else
  [ -f /tmp/petget-proc/petget/current-repo-triad.previous ] && mv -f /tmp/petget-proc/petget/current-repo-triad.previous /tmp/petget-proc/petget/current-repo-triad #120504
  exit
 fi
fi
...
touch /tmp/petget-proc/petget_missing_dbentries-${DB_FILE} #create if doesn't exist.
mPATTERN='^'"$TREE1"'|'
if [ "`grep "$mPATTERN" /tmp/petget-proc/petget_missing_dbentries-${DB_FILE}`" = "" ];then
 echo "$DB_ENTRY" >> /tmp/petget-proc/petget_missing_dbentries-${DB_FILE}
fi

#now do the actual install...
PASSEDPRM=""
[ "`echo "$RETPARAMS" | grep '^EXIT' | grep 'BUTTON_PKGS_DOWNLOADONLY'`" != "" ] && PASSEDPRM="DOWNLOADONLY" && touch /tmp/petget-proc/manual_pkg_download
...
[ "`cat /tmp/petget-proc/petget-installed-pkgs-log | grep -o 'CATEGORY' | grep -v 'none'`" = "" ] && INSTALLEDCAT="none"
...
iUPDATE='no'
for iONE in `cat /tmp/petget-proc/petget_missing_dbentries-* | cut -f 1 -d '|' | tr '\n' ' '`
do
 if [ -f /root/.packages/package-files/${iONE}.files ]; then
  [ "`grep 'usr/share/icons/hicolor' /root/.packages/package-files/${iONE}.files`" != "" ] \
   && echo yes >> /tmp/petget-proc/iUPDATE
 fi
done
if [ "$UPDATE_MENUS" = "yes" ]; then
 if [ "$(grep yes /tmp/petget-proc/iUPDATE)" != "" ]; then \
  gtk-update-icon-cache -f /usr/share/icons/hicolor/
  rm -f /tmp/petget-proc/iUPDATE
 fi
fi
...
[ ! -f /tmp/petget-proc/install_quietly ] && kill $X3PID || echo
...
PKGS="`cat /tmp/petget-proc/petget_missing_dbentries-* | cut -f 1 -d '|' | tr '\n' '|'`"
