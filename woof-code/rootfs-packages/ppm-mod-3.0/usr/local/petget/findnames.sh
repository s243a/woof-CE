...
#120504 some files moved into /tmp/petget-proc/petget
...
CURRENTREPO="`cat /tmp/petget-proc/petget/current-repo-triad`" #search here first.
ALLACTIVEREPOS="`cat /tmp/petget-proc/petget_active_repo_list`"
SEARCH_REPOS_FLAG=$1

if [ "$(cat /var/local/petget/ui_choice 2>/dev/null)" = "Classic" ]; then
 #120504 ask which repos...
 export ASKREPO_DIALOG="<window title=\"$(gettext 'PPM: search')\" icon-name=\"gtk-about\" resizable=\"false\">
 <vbox>
...
FNDIT=no
echo -n "" > /tmp/petget-proc/petget/filterpkgs.results
...
#  ALPHAPRE="`cat /tmp/petget-proc/petget_pkg_first_char`"
  #this is read when update TREE1 in pkg_chooser.sh...
  #echo "$FNDENTRIES" | cut -f 1,10 -d '|' > /tmp/petget-proc/petget/filterpkgs.results
  ...
  if  [ "$FPR" = "|${ONEREPO}" ];then
   ...
   echo "$FPR" >> /tmp/petget-proc/petget/filterpkgs.results #120504 append repo-triad each line.
  fi
  FNDIT=yes
#120504  break
 fi
...
if [ "$FNDIT" = "yes" ];then
 ...
 BADKERNPTNS="`grep -o '\-k2\.6\.[^-|a-zA-Z]*' /tmp/petget-proc/petget/filterpkgs.results | cut -f 1 -d '|' | grep -v "$GOODKERNPTN" | sed -e 's%$%-%' -e 's%\.%\\\.%g' -e 's%\-%\\\-%g'`" #ex: \-k2\.6\.32\.28\-
 if [ "$BADKERNPTNS" ];then
  echo "$BADKERNPTNS" >> /tmp/petget-proc/petget_badkernptns
  grep -v -f /tmp/petget-proc/petget_badkernptns /tmp/petget-proc/petget/filterpkgs.results > /tmp/petget-proc/petget/filterpkgs.resultsxxx
  mv -f /tmp/petget-proc/petget/filterpkgs.resultsxxx /tmp/petget-proc/petget/filterpkgs.results
 fi
fi

if [ "$FNDIT" = "no" ];then
 #120909 these files may have been created at previous search, it will upset show_installed_version_diffs.sh if still exist...
 [ -f /tmp/petget-proc/petget/filterpkgs.results.installed ] && rm -f /tmp/petget-proc/petget/filterpkgs.results.installed
 [ -f /tmp/petget-proc/petget/filterpkgs.results.notinstalled ] && rm -f /tmp/petget-proc/petget/filterpkgs.results.notinstalled
 ...
 if [ -f /tmp/petget-proc/petget_installed_patterns_all ];then #precaution.
  grep -f /tmp/petget-proc/petget_installed_patterns_all -v /tmp/petget-proc/petget/filterpkgs.results > /tmp/petget-proc/petget/filterpkgs.results.notinstalled
  grep -f /tmp/petget-proc/petget_installed_patterns_all /tmp/petget-proc/petget/filterpkgs.results > /tmp/petget-proc/petget/filterpkgs.results.installed
  cp -f /tmp/petget-proc/petget/filterpkgs.results.notinstalled /tmp/petget-proc/petget/filterpkgs.results
  if [ -s /tmp/petget-proc/petget/filterpkgs.results.installed ];then
   ...
   #120908 now have version field (in field #3), ex: xserver-xorg-video-radeon_6.14.99|xserver-xorg-video-radeon|6.14.99|BuildingBlock|X.Org X server -- AMD/ATI Radeon display driver|puppy-noarch-official|
    sed -e 's%|%ONEPIPECHAR%' -e 's%|%ONEPIPECHAR%' -e 's%|[^|]*%|complete%' -e 's%|complete|%|complete|(ALREADY INSTALLED) %' -e 's%ONEPIPECHAR%|%g' /tmp/petget-proc/petget/filterpkgs.results.installed >> /tmp/petget-proc/petget/filterpkgs.results
 ...
 cut -f 1,4,5,6,7 -d '|' /tmp/petget-proc/petget/filterpkgs.results > /tmp/petget-proc/petget/filterpkgs.results1 #note, retain | on end.
 mv -f /tmp/petget-proc/petget/filterpkgs.results1 /tmp/petget-proc/petget/filterpkgs.results
 ...
 #...main gui will read /tmp/petget-proc/petget/filterpkgs.results.post (actually that happens in ui_Classic or ui_Ziggy, which is included in pkg_chooser.sh).
 ...
 if [ "$SEARCH_REPOS_FLAG" = "all" ];then
  ...
  #120811 format in /tmp/petget-proc/petget/filterpkgs.results.post now: pkgname|subcategory|description|dbfile, 
  # ex: htop-0.9-i486|System|View Running Processes|puppy-wary5-official (previously was: pkgname|description|dbfile)
  POSTPROCLIST="`printcols /tmp/petget-proc/petget/filterpkgs.results.post 1 2 4 3 4 | sed -e 's%|%FIRSTBARCHAR%' -e 's%|%SECBARCHAR[%' -e 's%|%] %' -e 's%FIRSTBARCHAR%|%' -e 's%SECBARCHAR%|%'`"
  echo "$POSTPROCLIST" > /tmp/petget-proc/petget/filterpkgs.results.post
  #ex line: abiword-1.2.3|[puppy-4-official] Abiword word processor|puppy-4-official|
 fi
 
 if [ -f /tmp/petget-proc/petget/filterpkgs.results.post.tmp ]; then
 rm -f /tmp/petget-proc/petget/filterpkgs.results.post.tmp
 fi
 ...
 while IFS='' read -r line || [[ -n $line ]]
	do
		...
		echo "$line$pkgname#$reponame" >> /tmp/petget-proc/petget/filterpkgs.results.post.tmp
		
 done < /tmp/petget-proc/petget/filterpkgs.results.post
	
 mv -f /tmp/petget-proc/petget/filterpkgs.results.post.tmp /tmp/petget-proc/petget/filterpkgs.results.post
