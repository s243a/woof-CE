...
cp -f /tmp/petget-proc/petget/filterpkgs.results /tmp/petget-proc/petget/filterpkgs.results.post

#120525 quick filtering but not perfect...
#PETs: _DEV _DOC _NLS  
#Ubuntu,Debian DEBs: -dev_ -doc_ -docs_ -langpack -lang-
#Mageia RPMs: -devel- -doc-
sed -i -e '/-dbg_/d' /tmp/petget-proc/petget/filterpkgs.results.post #120525 always take out the debug pkgs.
[ "$DEF_CHK_DEV" = "false" ] && sed -i -e '/_DEV/d' -e '/-dev_/d' -e '/-devel-/d' /tmp/petget-proc/petget/filterpkgs.results.post
[ "$DEF_CHK_DOC" = "false" ] && sed -i -e '/_DOC/d' -e '/-doc_/d' -e '/-docs_/d' -e '/-doc-/d' /tmp/petget-proc/petget/filterpkgs.results.post
[ "$DEF_CHK_NLS" = "false" ] && sed -i -e '/_NLS/d' -e '/-langpack/d' -e '/-lang-/d' /tmp/petget-proc/petget/filterpkgs.results.post
#120504b fix filtering out _EXE... 120515 must escape the dashes...
if [ "$DEF_CHK_EXE" = "false" ];then
 grep -E '_DEV|_DOC|_NLS|\-dev_|\-doc_|\-docs_|\-langpack|\-lang\-|\-devel\-|\-doc\-' /tmp/petget-proc/petget/filterpkgs.results.post > /tmp/petget-proc/petget/filterpkgs.results.post.tmp
 mv -f /tmp/petget-proc/petget/filterpkgs.results.post.tmp /tmp/petget-proc/petget/filterpkgs.results.post
fi

sed -i 's%;%-%' /tmp/petget-proc/petget/filterpkgs.results.post #make names correspond to /usr/share/icons/hicolor/scalable/categogies/
