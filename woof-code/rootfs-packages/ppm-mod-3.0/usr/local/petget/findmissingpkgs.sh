...
#/tmp/petget-proc/petget_pkg_name_aliases_patterns is written by pkg_chooser.sh.
...
echo "$PKGDEPS_PATTERNS" > /tmp/petget-proc/petget_pkg_deps_patterns #ex line, mageia: |libdbus-glib-1_2|
...
echo "$PKGDEPS_PATTERNS_WITHVER" > /tmp/petget-proc/petget_pkg_deps_patterns_with_versioning #ex line, mageia: |libdbus-glib-1_2|ge2.3.6|

#110706 mageia, a dep "libdbus-glib-1_2" must be located in variable PKG_ALIASES_INSTALLED (in file PKGS_MANAGEMENT)...
#/tmp/petget-proc/petget_pkg_name_aliases_patterns[_raw] created in check_deps.sh
for ONEALIAS in `cat /tmp/petget-proc/petget_pkg_name_aliases_patterns_raw | tr '\n' ' ' | tr ',' ' '` #ex: |cxxlibs|,|glibc.*|,|libc\-.*|
do
 FNDDEPSPTNS="`grep "$ONEALIAS" /tmp/petget-proc/petget_pkg_deps_patterns`"
 if [ "$FNDDEPSPTNS" != "" ];then
  echo "$FNDDEPSPTNS" >> /tmp/petget-proc/petget_pkg_name_aliases_patterns
 fi
done

#need patterns of all installed pkgs...
#100711 /tmp/petget-proc/petget_installed_patterns_system is created in pkg_chooser.sh.
  ...
  ubuntu|debian|devuan|raspbian)
   ...
   if [ $MODIF1 -gt $MODIF2 ];then
    ...
    if [ "$INSTALLED_PTNS_PET" != "" ];then
     ...
     echo "$xINSTALLED_PTNS_PET" > /tmp/petget-proc/petget/fmp_xipp
     INSTALLED_ALT_NAMES="$(grep --no-filename -f /tmp/petget-proc/petget/fmp_xipp /root/.packages/repo/Packages-${DISTRO_BINARY_COMPAT}-${DISTRO_COMPAT_VERSION}-* | cut -f 2 -d '|')"
...
INSTALLEDALIASES="`grep --file=/tmp/petget-proc/petget_installed_patterns_all /tmp/petget-proc/petget_pkg_name_aliases_patterns | tr ',' '\n'`"
[ "$INSTALLEDALIASES" ] && echo "$INSTALLEDALIASES" >> /tmp/petget-proc/petget_installed_patterns_all

#110706 mageia, a dep "libdbus-glib-1_2" must be located in variable PKG_ALIASES_INSTALLED (in file PKGS_MANAGEMENT)...
#/tmp/petget-proc/petget_pkg_name_aliases_patterns[_raw] created in check_deps.sh, pkg_chooser.sh
for ONEALIAS in `cat /tmp/petget-proc/petget_pkg_name_aliases_patterns_raw | tr '\n' ' ' | tr ',' ' '` #ex: |cxxlibs|,|glibc.*|,|libc\-.*|
do
 FNDPTN="`grep "$ONEALIAS" /tmp/petget-proc/petget_installed_patterns_all`"
 if [ "$FNDPTN" ];then
  FNDDEPPTN="`grep "$ONEALIAS" /tmp/petget-proc/petget_pkg_deps_patterns`"
  [ "$FNDDEPPTN" ] && echo "$FNDDEPPTN" >> /tmp/petget-proc/petget_installed_patterns_all
 fi
done

#100126 some names to ignore, as most likely already installed...
#/tmp/petget-proc/petget_pkg_name_ignore_patterns is created in pkg_choose.sh
cat /tmp/petget-proc/petget_pkg_name_ignore_patterns >> /tmp/petget-proc/petget_installed_patterns_all

#clean it up...
grep -v '^$' /tmp/petget-proc/petget_installed_patterns_all > /tmp/petget-proc/petget_installed_patterns_all-tmp
mv -f /tmp/petget-proc/petget_installed_patterns_all-tmp /tmp/petget-proc/petget_installed_patterns_all

#remove installed pkgs from the list of dependencies...
MISSINGDEPS_PATTERNS="`grep --file=/tmp/petget-proc/petget_installed_patterns_all -v /tmp/petget-proc/petget_pkg_deps_patterns | grep -v '^$'`"
echo "$MISSINGDEPS_PATTERNS" > /tmp/petget-proc/petget_missingpkgs_patterns #can be read by dependencies.sh, find_deps.sh.
...
#  |mesa-lib|
#  |qt|
#/tmp/petget-proc/petget_installed_patterns_all (read in dependencies.sh) has a list of already installed
...
#110722
MISSINGDEPS_PATTERNS_WITHVER="`grep --file=/tmp/petget-proc/petget_missingpkgs_patterns /tmp/petget-proc/petget_pkg_deps_patterns_with_versioning | grep -v '^$'`"
echo "$MISSINGDEPS_PATTERNS_WITHVER" > /tmp/petget-proc/petget_missingpkgs_patterns_with_versioning #can be read by dependencies.sh, find_deps.sh.
...
#120831 npierce, jemimah: dependencies.sh now using /tmp/petget-proc/petget_missingpkgs_patterns_with_versioning.
