...
#/tmp/petget-proc/petget_installpreview_pkgname (writen in installpreview.sh) has name of package being
#  previewed prior to installation. ex: abiword-1.2.3
#/tmp/petget-proc/petget/current-repo-triad has the repository that installing from (written in pkgchooser.sh).
#  ex: slackware-12.2-slacky
#  ...full package database file is /root/.packages/repo/Packages-slackware-12.2-slacky
#/tmp/petget-proc/petget_missingpkgs_patterns (written in findmissingpkgs.sh) has a list of missing dependencies, format ex:
#  |kdebase|
#  |kdelibs|
#  |mesa-lib|
#  |qt|
#  ...that is, pkg name-only with vertical-bars on both ends, one name per line.
#/tmp/petget-proc/petget_installed_patterns_all (writen in findmissingpkgs.sh) has a list of already installed
...
#120831 npierce, jemimah: findmissingpkgs.sh creates /tmp/petget-proc/petget_missingpkgs_patterns and /tmp/petget-proc/petget_missingpkgs_patterns_with_versioning, need to use latter here.
...
ALLINSTALLEDPKGS="`cat /tmp/petget-proc/petget_installed_patterns_all`"
TREE1="`cat /tmp/petget-proc/petget_installpreview_pkgname`"


#this is the db of the main pkg...
DB_MAIN="${PREPATH}Packages-`cat /tmp/petget-proc/petget/current-repo-triad`" #ex: Packages-slackware-12.2-official 110723
...
SIZE2=0 #120907
echo -n "" > /tmp/petget-proc/petget_missingpkgs_patterns_acc #120903
echo -n "" > /tmp/petget-proc/petget_missingpkgs_patterns_acc0 #120903
cp -f /tmp/petget-proc/petget_missingpkgs_patterns /tmp/petget-proc/petget_missingpkgs_patternsx
echo "$(gettext 'HIERARCHY OF MISSING DEPENDENCIES OF PACKAGE') $TREE1" > /tmp/petget-proc/petget_deps_visualtreelog #w017
echo "$(gettext "Format of each line: 'a-missing-dependent-pkg: missing dependencies of a-missing-dependent-pkg'")" >> /tmp/petget-proc/petget_deps_visualtreelog #w017
...
do
 ...
 echo "" >> /tmp/petget-proc/petget_deps_visualtreelog #w017
 echo -n "" > /tmp/petget-proc/petget_missingpkgs_patterns2
 for depPATTERN in `cat /tmp/petget-proc/petget_missingpkgs_patternsx`
 do
  ...
  do
   ...
   if [ "$DB_dependencies" != "" ];then
    ...
    echo "$ALLDEPS_PATTERNS" > /tmp/petget-proc/petget_subpkg_deps_patterns
    cp -f /tmp/petget-proc/petget_installed_patterns_all /tmp/petget-proc/petget_installed_patterns_allxx #120907
    cat /tmp/petget-proc/petget_missingpkgs_patterns_acc >> /tmp/petget-proc/petget_installed_patterns_allxx #120907 greatly speeds search, improves hierarchy view.
    MISSINGDEPS_PATTERNS="`grep --file=/tmp/petget-proc/petget_installed_patterns_allxx -v /tmp/petget-proc/petget_subpkg_deps_patterns`"
    echo "$MISSINGDEPS_PATTERNS" >> /tmp/petget-proc/petget_missingpkgs_patterns2
    #w017 log a visual tree...
    MISSDEPSLIST="`echo "$MISSINGDEPS_PATTERNS" | sed -e 's%|%%g' | tr '\n' ' '`"
    case $ONELEVEL in
     1)  echo "$ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     2)  echo "    $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     3)  echo "        $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     4)  echo "            $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     5)  echo "                $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     6)  echo "                    $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     7)  echo "                        $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     8)  echo "                            $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     9)  echo "                            $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     10) echo "                                $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
     11) echo "                                    $ONEDEP: $MISSDEPSLIST" >> /tmp/petget-proc/petget_deps_visualtreelog ;;
    esac
    break
   fi
  done
 done
 cp -f /tmp/petget-proc/petget_missingpkgs_patterns_acc /tmp/petget-proc/petget_missingpkgs_patterns_acc-prev #120907
 sort -u /tmp/petget-proc/petget_missingpkgs_patterns2 > /tmp/petget-proc/petget_missingpkgs_patternsx0
 grep -v '^$' /tmp/petget-proc/petget_missingpkgs_patternsx0 > /tmp/petget-proc/petget_missingpkgs_patternsx
 cat /tmp/petget-proc/petget_missingpkgs_patternsx >> /tmp/petget-proc/petget_missingpkgs_patterns_acc0 #accumulate them.
 sort -u /tmp/petget-proc/petget_missingpkgs_patterns_acc0 > /tmp/petget-proc/petget_missingpkgs_patterns_acc
 #120907 get out of loop when no more missing deps found...
 SIZE1=$(cat /tmp/petget-proc/petget_missingpkgs_patterns_acc-prev | wc -l)
 SIZE2=$(cat /tmp/petget-proc/petget_missingpkgs_patterns_acc | wc -l)
 [ $SIZE1 -eq $SIZE2 ] && break
done
[ ! -f /tmp/petget-proc/install_quietly ] && pupkill $XXPID

#120903 bring back the versioning info from level1 (/tmp/petget-proc/petget_missingpkgs_patterns_with_versioning is created in findmissingpkgs.sh)...
#restore the format ex |abiword|gt3.4| becomes |abiword&gt3.4| ...
grep -v --file=/tmp/petget-proc/petget_missingpkgs_patterns /tmp/petget-proc/petget_missingpkgs_patterns_acc > /tmp/petget-proc/petget_missingpkgs_patterns_accx #make sure that 2nd file does not have any level1 deps.
mv -f /tmp/petget-proc/petget_missingpkgs_patterns_accx /tmp/petget-proc/petget_missingpkgs_patterns_acc
sed -e 's%|%\&%g' -e 's%^\&%|%' -e 's%\&$%|%' /tmp/petget-proc/petget_missingpkgs_patterns_with_versioning >> /tmp/petget-proc/petget_missingpkgs_patterns_acc
sort -u /tmp/petget-proc/petget_missingpkgs_patterns_acc > /tmp/petget-proc/petget_missingpkgs_patternsx
mv -f /tmp/petget-proc/petget_missingpkgs_patternsx /tmp/petget-proc/petget_missingpkgs_patterns_and_versioning_level1
...
#111107 01micko: fix for '||' messing things up...
for depPATTERN in `grep '[a-zA-Z]' /tmp/petget-proc/petget_missingpkgs_patterns_and_versioning_level1` #ex depPATTERN=|kdelibs| ex2: |kdelibs&gt2.3.6|. 120221 jemimah. 120903 versioning.
do
   ...
   do
    ...
    if [ "$depVERSIONING" ];then #110722
     ...
     if [ "$condFLG" = "good" ];then
      echo "$DB_ENTRY" >> /tmp/petget-proc/petget_missing_dbentries-${DBFILE}-2
      break 2
     fi
    else
     echo "$DB_ENTRY" >> /tmp/petget-proc/petget_missing_dbentries-${DBFILE}-2
...
do
 DBFILE="`basename $ONEREPODB`" #ex: Packages-slackware-12.2-official
 if [ -f /tmp/petget-proc/petget_missing_dbentries-${DBFILE}-2 ];then
  sort -u /tmp/petget-proc/petget_missing_dbentries-${DBFILE}-2 > /tmp/petget-proc/petget_missing_dbentries-${DBFILE}
  rm -f /tmp/petget-proc/petget_missing_dbentries-${DBFILE}-2
 fi
done

# Give priority to Slackware patches over official
if [ "`echo $DISTRO_FILE_PREFIX | grep slacko`" != "" ]; then
 PATCHES=/tmp/petget-proc/petget_missing_dbentries-Packages-*lackware*-patches
 OFFICIAL=/tmp/petget-proc/petget_missing_dbentries-Packages-*lackware*-official
 SALIX=/tmp/petget-proc/petget_missing_dbentries-Packages-*lackware*-salix
 SLACKY=/tmp/petget-proc/petget_missing_dbentries-Packages-*lackware*-slacky
...
[ ! -f /tmp/petget-proc/install_quietly ] && kill $X1PID || exit 0
