...
clean_up () {
 if [ "$(ls /tmp/petget-proc/*_pet{,s}_quietly /tmp/petget-proc/install_classic 2>/dev/null |wc -l)" -eq 1 ]; then
  for MODE in $(ls /tmp/petget-proc/*_pet{,s}_quietly /tmp/petget-proc/install_classic)
  do
   mv $MODE $MODE.bak
  done
 fi
 mv /tmp/petget-proc/install_quietly /tmp/petget-proc/install_quietly.bak
 echo -n > /tmp/petget-proc/pkgs_to_install
 rm -f /tmp/petget-proc/{install,remove}{,_pets}_quietly 2>/dev/null
 rm -f /tmp/petget-proc/install_classic 2>/dev/null
 
 rm -f /tmp/petget-proc/res-dload-gt 2>/dev/null
 rm -f /tmp/petget-proc/res-dload-lt 2>/dev/null
 rm -f /tmp/petget-proc/res-dload-same 2>/dev/null
 
 rm -f /tmp/petget-proc/download_pets_quietly 2>/dev/null
 rm -f /tmp/petget-proc/download_only_pet_quietly 2>/dev/null
 rm -f /tmp/petget-proc/pkgs_left_to_install 2>/dev/null
 rm -f /tmp/petget-proc/pkgs_to_install_done 2>/dev/null
 rm -f /tmp/petget-proc/overall_pkg_size* 2>/dev/null
 rm -f /tmp/petget-proc/overall_dependencies 2>/dev/null
 rm -f /tmp/petget-proc/mode_changed 2>/dev/null
 rm -f /tmp/petget-proc/force*_install 2>/dev/null
 rm -f /tmp/petget-proc/pkgs_to_install_done 2>/dev/null
 rm -f /tmp/petget-proc/pgks_really_installed 2>/dev/null
 rm -f /tmp/petget-proc/pgks_failed_to_install 2>/dev/null
 rm -f /tmp/petget-proc/overall_petget_missingpkgs_patterns.txt 2>/dev/null
 rm -f /tmp/petget-proc/overall_missing_libs.txt 2>/dev/null
 rm -f /tmp/petget-proc/overall_install_report 2>/dev/null
 rm -f /tmp/petget-proc/pkgs_to_install_bar 2>/dev/null
 rm -f /tmp/petget-proc/manual_pkg_download 2>/dev/null
 rm -f /tmp/petget-proc/ppm_reporting 2>/dev/null
 rm -f /tmp/petget-proc/pkgs_DL_BAD_LIST 2>/dev/null
 rm -rf /tmp/petget-proc/PPM_LOGs/ 2>/dev/null
 mv $MODE.bak $MODE
 mv /tmp/petget-proc/install_quietly.bak /tmp/petget-proc/install_quietly
}
export -f clean_up

report_results () {
 # Info source files
 touch /tmp/petget-proc/ppm_reporting # progress bar flag
 /usr/local/petget/finduserinstalledpkgs.sh #make sure...
 sync
 rm -f /tmp/petget-proc/pgks_really_installed 2>/dev/null
 rm -f /tmp/petget-proc/pgks_failed_to_install 2>/dev/null
 for LINE in $(cat /tmp/petget-proc/pkgs_to_install_done  | cut -f 1 -d '|' | sort | uniq)
 do
  if [ -f /tmp/petget-proc/download_pets_quietly -o -f /tmp/petget-proc/download_only_pet_quietly \
   -o -f /tmp/petget-proc/manual_pkg_download ];then
   if [ -f /root/.packages/download_path ];then
    . /root/.packages/download_path
    DOWN_PATH="$DL_PATH"
   else
   ...
   [ "$REALLY" -a "$(grep $LINE /tmp/petget-proc/pkgs_DL_BAD_LIST 2>/dev/null | sort | uniq )" != "" ] && \
    REALLY='' && PREVINST="$(gettext 'was previously downloaded')"
  else
   PREVINST=''
   REALLY=$(grep $LINE /tmp/petget-proc/petget/installedpkgs.results)
   [ "$(grep $LINE /tmp/petget-proc/pgks_failed_to_install_forced 2>/dev/null | sort | uniq )" != "" -o \
    "$(grep $LINE /tmp/petget-proc/pkgs_DL_BAD_LIST 2>/dev/null | sort | uniq )" != "" ] \
    && REALLY='' && PREVINST="$(gettext 'was already installed')"
  fi
  if [ "$REALLY" != "" ]; then
   echo $LINE >> /tmp/petget-proc/pgks_really_installed
  else
   echo $LINE $PREVINST >> /tmp/petget-proc/pgks_failed_to_install
  fi
 done
 rm -f /tmp/petget-proc/pgks_failed_to_install_forced

 if [ -f /tmp/petget-proc/pgks_really_installed ]; then
 INSTALLED_PGKS="$(</tmp/petget-proc/pgks_really_installed)"
 PKGS_SUCCESS=$(wc -l < /tmp/petget-proc/pgks_really_installed)
 else
 INSTALLED_PGKS=''
 PKGS_SUCCESS=0
 fi
 
 if [ -f /tmp/petget-proc/pgks_failed_to_install ]; then
 FAILED_TO_INSTALL="$(</tmp/petget-proc/pgks_failed_to_install)"
 PKGS_FAILED=$(wc -l < /tmp/petget-proc/pgks_failed_to_install)
 else
 FAILED_TO_INSTALL=''
 PKGS_FAILED=0
 fi
 
 #MISSING_PKGS=$(cat /tmp/petget-proc/overall_petget_missingpkgs_patterns.txt |sort|uniq )
 MISSING_LIBS=$(cat /tmp/petget-proc/overall_missing_libs.txt 2>/dev/null | tr ' ' '\n' | sort | uniq )
 NOT_IN_PATH_LIBS=$(cat /tmp/petget-proc/overall_missing_libs_hidden.txt 2>/dev/null | tr ' ' '\n' | sort | uniq )
 cat << EOF > /tmp/petget-proc/overall_install_report
...
${MISSING_LIBS}"
  PKGS_LIBS=$(wc -l < /tmp/petget-proc/overall_missing_libs.txt)
  
  if [ -f /tmp/petget-proc/overall_missing_libs_hidden.txt ]; then
  HIDDEN_LIBS=$(wc -l < /tmp/petget-proc/overall_missing_libs_hidden.txt)
  else
  HIDDEN_LIBS=0
  fi
...
${NOT_IN_PATH_LIBS}"
 ...
 <hbox space-expand="false" space-fill="false">
	<button>
	  ...
	  <action>defaulttextviewer /tmp/petget-proc/overall_install_report &</action>
	 </button>
	 <button ok></button>
 ...
 echo 100 > /tmp/petget-proc/petget/install_status_percent
}
export -f report_results

check_total_size () {
 rm -f /tmp/petget-proc/petget_deps_visualtreelog 2>/dev/null
 rm -f /tmp/petget-proc/petget_frame_cnt 2>/dev/null
 rm -f /tmp/petget-proc/petget_missingpkgs_patterns{2,_acc,_acc0,_acc-prev,x0,_and_versioning_level1} 2>/dev/null
 rm -f /tmp/petget-proc/petget_moreframes 2>/dev/null
 rm -f /tmp/petget-proc/petget_tabs 2>/dev/null
 rm -f /tmp/petget-proc/pkgs_to_install_bar 2>/dev/null
 #required size
 NEEDEDK_PLUS=$( expr $(awk '{ sum += $1 } END { print sum }' /tmp/petget-proc/overall_pkg_size)) 
 [ -f /tmp/petget-proc/overall_pkg_size_RMV ] && \
  NEEDEDK_MINUS=$( expr $(awk '{ sum += $1 } END { print sum }' /tmp/petget-proc/overall_pkg_size_RMV)) \
  || NEEDEDK_MINUS=0
 ...
 if [ -f /tmp/petget-proc/download_pets_quietly -o -f /tmp/petget-proc/download_only_pet_quietly ]; then
 ...
 PACKAGES=$(cat /tmp/petget-proc/pkgs_to_install | cut -f 1 -d '|')
 DEPENDENCIES=$(cat /tmp/petget-proc/overall_dependencies 2>/dev/null | sort | uniq)
 [ "$AVAILABLE" = "0" -o  "$AVAILABLE" = "" ] && echo "No space left on device. Exiting" \
	> /tmp/petget-proc/petget/install_status && clean_up && exit 0
 #statusbar in main gui
 PERCENT=$((${NEEDEDK}*100/${AVAILABLE}))
 [ $PERCENT -gt 99 ] && PERCENT=99
 if [ -s /tmp/petget-proc/overall_pkg_size ] && [ $PERCENT = 0 ]; then PERCENT=1; fi
 echo "$PERCENT" > /tmp/petget-proc/petget/install_status_percent
 if [ "$(cat /tmp/petget-proc/pkgs_to_install /tmp/petget-proc/overall_dependencies 2>/dev/null)" = "" ]; then
  echo "" > /tmp/petget-proc/petget/install_status
 else
  cat /tmp/petget-proc/pkgs_to_install | cut -f1 -d '|' > /tmp/petget-proc/pkgs_to_install_bar
  if [ -f /tmp/petget-proc/install_pets_quietly -o -f /tmp/petget-proc/install_classic ]; then
   if [ "$(cat /var/local/petget/nd_category 2>/dev/null)" != "true" ]; then
  ...
  echo "$(gettext 'Packages (with deps)'): $(cat /tmp/petget-proc/pkgs_to_install_bar /tmp/petget-proc/overall_dependencies 2>/dev/null |sort | uniq | wc -l)    -   $(gettext 'Required space') ${BARMSG}: ${BARNEEDEDK}MB   -   $(gettext 'Available'): ${AVAILABLE}MB" > /tmp/petget-proc/petget/install_status
 fi
 ...
 if [ "$NEEDEDK" -ge "$AVAILABLE" -o "$NEEDEDKDOWN" -ge "$SAVEAVAILABLE" ]; then
  ...
  killall yaf-splash
  if [ ! -f /tmp/petget-proc/install_classic ]; then
   echo "" > /tmp/petget-proc/petget/install_status
   echo 0 > /tmp/petget-proc/petget/install_status_percent
   if [ "$(ls /tmp/petget-proc/*_pet{,s}_quietly /tmp/petget-proc/install_classic |wc -l)" -eq 1 ]; then
	for MODE in $(ls /tmp/petget-proc/*_pet{,s}_quietly /tmp/petget-proc/install_classic)
	do
	 mv $MODE $MODE.bak
	done
...
status_bar_func () {
 while $1 ; do
  TOTALPKGS=$(cat /tmp/petget-proc/pkgs_to_install_bar /tmp/petget-proc/overall_dependencies 2>/dev/null |sort | uniq | wc -l)
  DONEPGKS=$(cat /tmp/petget-proc/overall_package_status_log 2>/dev/null | wc -l)
  PERCENT=$( expr $DONEPGKS \* 100 \/ $TOTALPKGS )
  [ $PERCENT = 100 ] && PERCENT=99
  echo $PERCENT > /tmp/petget-proc/petget/install_status_percent
  sleep 0.3
  [ -f /tmp/petget-proc/ppm_reporting ] && break
 done
}
export -f status_bar_func
 
install_package () {
 [ "$(cat /tmp/petget-proc/pkgs_to_install)" = "" ] && exit 0
 cat /tmp/petget-proc/pkgs_to_install | tr ' ' '\n' > /tmp/petget-proc/pkgs_left_to_install
 rm -f /tmp/petget-proc/overall_package_status_log
 echo 0 > /tmp/petget-proc/petget/install_status_percent
 echo "$(gettext "Calculating total required space...")" > /tmp/petget-proc/petget/install_status
 ...
 while read LINE; do
   ...
   if [ -f /tmp/petget-proc/install_quietly ];then
    if [  "$(grep $TREE1 /root/.packages/user-installed-packages 2>/dev/null)" = "" \
     -a -f /tmp/petget-proc/install_pets_quietly ]; then
   ...
   sed -i "/$TREE1/d" /tmp/petget-proc/pkgs_left_to_install
 done < /tmp/petget-proc/pkgs_to_install
 sync
 report_results
 clean_up
}
...
recalculate_sizes () {
	if [ "$(grep changed /tmp/petget-proc/mode_changed 2>/dev/null)" != "" ]; then
		rm -f /tmp/petget-proc/overall_*
		for LINE in $(cat /tmp/petget-proc/pkgs_to_install)
		do
	...
	rm -f /tmp/petget-proc/mode_changed
}
...
case "$1" in
	...
	"$(gettext 'Auto install')")
		wait_func
		rm -f /tmp/petget-proc/install_pets_quietly
		rm -f /tmp/petget-proc/install_classic 2>/dev/null
		rm -f /tmp/petget-proc/download_pets_quietly 2>/dev/null
		rm -f /tmp/petget-proc/download_only_pet_quietly 2>/dev/null
		touch /tmp/petget-proc/install_quietly
		touch /tmp/petget-proc/install_pets_quietly
		cp -a /tmp/petget-proc/pkgs_to_install /tmp/petget-proc/pkgs_to_install_done
		VTTITLE=Installing
		export VTTITLE
		install_package
		unset VTTITLE
	...
	"$(gettext 'Step by step installation (classic mode)')")
		wait_func
		rm -f /tmp/petget-proc/install{,_pets}_quietly
		rm -f /tmp/petget-proc/download_pets_quietly 2>/dev/null
		rm -f /tmp/petget-proc/download_only_pet_quietly 2>/dev/null
