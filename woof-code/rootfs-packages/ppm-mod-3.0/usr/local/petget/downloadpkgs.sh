...
#The database entries for the packages to be installed are in /tmp/petget-proc/petget_missing_dbentries-*
#ex: /tmp/petget-proc/petget_missing_dbentries-Packages-slackware-12.2-official
...
#120927 want to translate "CATEGORY:" and "PACKAGE:" that are in /tmp/petget-proc/petget-installed-pkgs-log (see installpkg.sh).
...
echo -n "" > /tmp/petget-proc/petget-installed-pkgs-log

PKGCNT=0 ; FAILCNT=0 ; EXITVAL=0 #101118
for ONELIST in `ls -1 /tmp/petget-proc/petget_missing_dbentries-Packages-*` #ex: petget_missing_dbentries-Packages-puppy-quirky-official
do
 ...
 echo -n "" > /tmp/petget-proc/petget_repos
 ...
 case $LISTNAME in
  Packages-puppy-*) #a .pet pkg.
   ...
   do
    ...
    [ "`echo -n "$LISTNAME" | grep "$ONEPETREPO_3_PATTERN"`" != "" ] && echo "${ONEPETREPO_1_2}|${LISTNAME}" >> /tmp/petget-proc/petget_repos
  ...
  *) #a compat pkg.
   ...
   do
    ...
    [ "`echo "$LISTNAME" | grep "$PARTPKGDB"`" != "" ] && echo "${ONEURLENTRY_1_2}|${LISTNAME}" >> /tmp/petget-proc/petget_repos
   done
  ;;
 esac
 
 sort --key=1 --field-separator="|" --unique /tmp/petget-proc/petget_repos > /tmp/petget-proc/petget_repos-tmp
 mv -f /tmp/petget-proc/petget_repos-tmp /tmp/petget-proc/petget_repos
 
 #/tmp/petget-proc/petget_repos has a list of repos for downloading these packages.
 #now put up a window, request which url to use...
 
 LISTNAMECUT="`echo -n "$LISTNAME" | cut -f 2-9 -d '-'`" #take Packages- off.
 
 REPOBUTTONS=""
 for ONEREPOSPEC in `cat /tmp/petget-proc/petget_repos`
 do
...
<vbox>
 ...
 <frame ${LISTNAMECUT}>
 <vbox scrollable=\"true\" shadow-type=\"0\" hscrollbar-policy=\"2\" space-expand=\"true\" space-fill=\"true\" height=\"180\">
  ${REPOBUTTONS}
...
" 
 ...
 if [ "`echo "$RETPARAMS" | grep 'RADIO_URL_LOCAL' | grep 'true'`" != "" ];then
  ...
  DOWNLOADFROM="file://${LOCALDIR}"
	  ...
	  if [ ! -f /tmp/petget-proc/install_quietly ]; then
	   ...
	   DOWNLOADFROM="`cat /tmp/petget-proc/petget_repos | grep "$URL_BASIC" | head -n 1 | cut -f 2 -d '|'`"
	  else
		DOWNLOADFROM=""
	   #DOWNLOADFROM="`awk '{ if (NR==1) print $0 }' /tmp/petget-proc/petget_repos | cut -f 2 -d '|'`"
	   #DOWNLOADFROM_ALT="`awk '{ if (NR==2) print $0 }' /tmp/petget-proc/petget_repos | cut -f 2 -d '|'`"
	  fi
 fi
 ...
 if [ ! -f /tmp/petget-proc/install_quietly ];then
  ...
  echo "$(gettext 'Testing that packages exist in repository')" > /tmp/petget-proc/petget/install_status
 fi
 DL_BAD_LIST=''
  ...
  if [ "`echo "$RETPARAMS" | grep 'RADIO_URL_LOCAL' | grep 'true'`" != "" ];then
   if [ ! -f ${LOCALDIR}/${ONEPKGNAME} ];then #121130 fix.
    [ ! -f ./${ONEPKGNAME} ] && DL_BAD_LIST="${DL_BAD_LIST} ${ONEPKGNAME}"
	   ...
	   if [ ! -f /tmp/petget-proc/install_quietly ]; then
		LANG=C wget -4 -t 2 -T 20 --waitretry=20 --spider -S "${DOWNLOADFROM}/${ONEFILE}" > /tmp/petget-proc/download_file_spider.log0 2>&1 #
			if [ $? -ne 0 ]; then
		   ...
		   for line1 in `cat /tmp/petget-proc/petget_repos`
		   do
		   
			if [ $PKGFOUND -eq 0 ]; then
				URL_PATH="`echo -n "$line1" | cut -f 2 -d '|'`"   
				LANG=C wget -4 -t 2 -T 20 --waitretry=20 --spider -S "${URL_PATH}/${ONEFILE}" > /tmp/petget-proc/download_file_spider.log0 2>&1 #
 ...
 [ ! -f /tmp/petget-proc/install_quietly ] && pupkill $testPID || echo
 if [ "$DL_BAD_LIST" ];then
  ...
  echo ${DL_BAD_LIST} >> /tmp/petget-proc/pkgs_DL_BAD_LIST
  exit 1
 fi
 ...
 do
  ...
  if [ "`echo "$RETPARAMS" | grep 'RADIO_URL_LOCAL' | grep 'true'`" != "" ];then
   ...
   echo "$(gettext 'downloading'): ${ONEFILE}" > /tmp/petget-proc/petget/install_status
  ...
  if [ -f "${DL_PATH}"/$DLPKG -a "$DLPKG" != "" ];then
   if [ "$PASSEDPARAM" = "DOWNLOADONLY" ];then
    echo "$(gettext 'Verifying'): ${ONEFILE}" > /tmp/petget-proc/petget/install_status
    /usr/local/petget/verifypkg.sh $DLPKG
   else
    echo "$(gettext 'Installing'): ${ONEFILE}" > /tmp/petget-proc/petget/install_status
    /usr/local/petget/installpkg.sh $DLPKG
    #...appends pkgname and category to /tmp/petget-proc/petget-installed-pkgs-log if successful.
   fi
   if [ $? -ne 0 ];then
    LASTPKG=$(tail -n 1 /tmp/petget-proc/pgks_failed_to_install_forced)
  ...
  [ "$PASSEDPARAM" != "DOWNLOADONLY" -a "$DL_SAVE_FLAG" != "true" \
   -a "$(grep ${DLPKG} /tmp/petget-proc/pkg_preexists)" = "" ] && rm -f $DLPKG 2>/dev/null
   rm -f /tmp/petget-proc/pkg_preexists 2>/dev/null
  else
...
if [ "$PASSEDPARAM" = "DOWNLOADONLY" -a ! -f /tmp/petget-proc/download_pets_quietly \
 -a ! -f /tmp/petget-proc/download_only_pet_quietly ];then
...
#installpkg.sh will have logged to /tmp/petget-proc/petget-installed-pkgs-log
if [ -s /tmp/petget-proc/petget-installed-pkgs-log ];then
 [ -f /tmp/petget-proc/install_quietly ] && FLAGPET='yes'
 ...
 INSTALLEDMSG="`cat /tmp/petget-proc/petget-installed-pkgs-log`" #ex line: "PACKAGE: langpack_ru-20120720 CATEGORY: Setup"
 ...
 ZINSTALLEDMSG="$(sed -e "$ZPTNDesktop" -e "$ZPTNSystem" -e "$ZPTNSetup" -e "$ZPTNUtility" -e "$ZPTNFilesystem" -e "$ZPTNGraphic" -e "$ZPTNDocument" -e "$ZPTNBusiness" -e "$ZPTNPersonal" -e "$ZPTNNetwork" -e "$ZPTNInternet" -e "$ZPTNMultimedia" -e "$ZPTNFun" -e "$ZPTNHelp" -e "$ZPTNnone" -e "$ZPTN1" -e "$ZPTN2" /tmp/petget-proc/petget-installed-pkgs-log)" #121011 more ptns.
...
" 
 ...
 if [ "$EXIT" = "BUTTON_TRIM_FAT" ];then
   ...
   do
    ...
    if [ "$ENTRY_LOCALE" != "" ];then
     if [ "`echo -n "$ONEFILE" | grep --extended-regexp '/locale/|/nls/|/i18n/' | grep -v -E "$elPATTERN"`" != "" ];then
      rm -f "$ONEFILE"
      grep -v "$ONEFILE" /root/.packages/package-files/${PKGNAME}.files > /tmp/petget-proc/petget_pkgfiles_temp
      mv -f /tmp/petget-proc/petget_pkgfiles_temp /root/.packages/package-files/${PKGNAME}.files
      continue
     fi
    fi
  ...
  [ ! -f /tmp/petget-proc/install_quietly ] && kill $X4PID || echo
 fi
