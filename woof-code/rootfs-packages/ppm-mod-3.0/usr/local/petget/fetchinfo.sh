...
case $DB_DISTRO in
 slackware)
  if [ ! -f /root/.packages/package-info/PACKAGES.TXT-${DB_SUB} ];then
#  /usr/lib/gtkdialog/box_splash -font "8x16" -outline 0 -margin 4 -text "Please wait, downloading database file to /root/.packages/PACKAGES.TXT-${DB_SUB}..." &
    ...
    #/usr/lib/gtkdialog/box_splash -close never -text "$(gettext 'Please wait, downloading database file to') /root/.packages/package-info/PACKAGES.TXT-${DB_SUB}..." &
    #X5PID=$!
   #fi
   
   cd /root/.packages/package-info
   case $DB_SUB in
    official)
     rxvt -bg blue -fg white -title "Downloading package info... DO NOT CLOSE" -geometry 80x6+50+50 -e wget http://slackware.cs.utah.edu/pub/slackware/slackware-${DB_RELEASE}/PACKAGES.TXT
    ;;
    slacky)
     rxvt -bg blue -fg white -title "Downloading package info... DO NOT CLOSE" -geometry 80x6+50+50 -e wget http://repository.slacky.eu/slackware-${DB_RELEASE}/PACKAGES.TXT
    ;;
   esac
   sync
   mv -f PACKAGES.TXT PACKAGES.TXT-${DB_SUB}
   #[ ! -f /tmp/petget-proc/install_quietly ] && kill $X5PID || echo
  fi
  
  cat /root/.packages/package-info/PACKAGES.TXT-${DB_SUB} | tr -s ' ' | sed -e 's% $%%' | tr '%' ' ' | tr '\n' '%' | sed -e 's/%%/@/g' | grep -o "PACKAGE NAME: ${DB_fullfilename}[^@]*" | tr '%' '\n' > /tmp/petget-proc/petget_slackware_pkg_extra_info
  
  INF=`cat /tmp/petget-proc/petget_slackware_pkg_extra_info`
  
  if [ "$INF" != "" ]; then
  sync
  nohup defaulttextviewer /tmp/petget-proc/petget_slackware_pkg_extra_info &
  else
  gxmessage "Package information currently unavailable" -title "Fetch information"
  fi
 ...
 ;;
 puppy|t2|gentoo)
   #HOMELINK="`grep 'Homepage:' /tmp/petget-proc/gethomepage_2 | grep -o 'href=".*' | cut -f 2 -d '"'`"
 ...
 scientific) #110523
  ###THIS IS INCOMPLETE###
  if [ ! -f /root/.packages/package-info/primary.xml ];then
   ...
   cd /root/.packages/package-info
   rxvt -bg blue -fg white -title "Downloading package info... DO NOT CLOSE" -geometry 80x6+50+50 -e wget http://ftp.scientificlinux.org/linux/scientific/${DISTRO_COMPAT_VERSION}/i386/os/repodata/primary.xml.gz
   sync
  ...
  nohup defaulttextviewer /root/.packages/package-info/primary.xml &
 ;;
esac
