...
wget -4 -t 2 -T 20 --waitretry=20 --spider -S --recursive --no-parent --no-directories -A 'devx_service_pack*.pet' "$URLSPEC" > /tmp/petget-proc/petget/devx_service_pack_probe 2>&1
...
FNDPETURLS="$(grep -o "$PTN1" /tmp/petget-proc/petget/devx_service_pack_probe | grep -v '\*' | tr '\n' ' ')"
...
#echo -n "" > /tmp/petget-proc/petget/devx_service_pack_hack
 ...
 if vercmp $ENDVER gt $INSTBIGGEST;then #precaution
  if vercmp $STARTVER eq $CURRVER;then #changed from 'le'
   if vercmp $ENDVER gt $CURRVER;then
    ...
    CNT=`expr $CNT + 1`
    #find the size...
    PARAS="$(cat /tmp/petget-proc/petget/devx_service_pack_probe | sed -e 's%^$%BLANKLINE%' | tr '\n' ' ' | tr -s ' ' | sed -e 's%BLANKLINE%\n%g')"
...
<radiobutton><label>${ABASE} SIZE:${LENK}K</label><variable>RADIOVAR_${CNT}_</variable></radiobutton>"
    #echo "PACKAGE: ${ABASE}  SIZE: ${LENK}K" >> /tmp/petget-proc/petget/devx_service_pack_hack
