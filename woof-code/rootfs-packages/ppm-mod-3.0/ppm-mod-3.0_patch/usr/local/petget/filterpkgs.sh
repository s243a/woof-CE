...
# ...this has written to /tmp/petget-proc/petget_pkg_first_char, ex: 'mn'
#filter category may be passed param to this script, ex: 'Document'
# or, /tmp/petget-proc/petget_filtercategory was written by pkg_chooser.sh.
#repo may be written to /tmp/petget-proc/petget/current-repo-triad by pkg_chooser.sh, ex: slackware-12.2-official
#/tmp/petget-proc/petget_pkg_name_aliases_patterns setup in pkg_chooser.sh, name aliases.
...
if [ $1 ];then #$1 exs: Document, Internet, Graphic, Setup, Desktop
 fltrCATEGORY="$1"
 echo "$1" > /tmp/petget-proc/petget_filtercategory
elif [ -f /tmp/petget-proc/petget_filtercategory ]; then #or, a selection was made in the main gui (pkg_chooser.sh)...
 fltrCATEGORY="`cat /tmp/petget-proc/petget_filtercategory`"
else
...
if [ ! -f /tmp/petget-proc/petget_fltrd_repo_${PKG_FIRST_CHAR}_${fltrCATEGORY}_${xDEFGUIFILTER}_Packages-${fltrREPO_TRIAD} ];then
 ...
 echo "$FLTRD_REPO" > /tmp/petget-proc/petget_fltrd_repo_${PKG_FIRST_CHAR}_${fltrCATEGORY}_${xDEFGUIFILTER}_Packages-${fltrREPO_TRIAD}
 #...file ex: /tmp/petget-proc/petget_fltrd_repo_a_Document_Packages-slackware-12.2-official
fi

#w480 extract names of packages that are already installed...
shortPATTERN="`cut -f 2 -d '|' /tmp/petget-proc/petget_fltrd_repo_${PKG_FIRST_CHAR}_${fltrCATEGORY}_${xDEFGUIFILTER}_Packages-${fltrREPO_TRIAD} | sed -e 's%^%|%' -e 's%$%|%'`"
echo "$shortPATTERN" > /tmp/petget-proc/petget_shortlist_patterns
INSTALLED_CHAR_CAT="`cat /root/.packages/layers-installed-packages /root/.packages/user-installed-packages | grep --file=/tmp/petget-proc/petget_shortlist_patterns`" #130511
...
if [ "$INSTALLED_CHAR_CAT" ];then #100711
 ...
 echo "$INSTALLED_PATTERNS" > /tmp/petget-proc/petget_installed_patterns
else
 echo -n "" > /tmp/petget-proc/petget_installed_patterns
fi

#packages may have different names, add them to installed list (refer pkg_chooser.sh)...
INSTALLEDALIASES="`grep --file=/tmp/petget-proc/petget_installed_patterns /tmp/petget-proc/petget_pkg_name_aliases_patterns | tr ',' '\n'`"
[ "$INSTALLEDALIASES" ] && echo "$INSTALLEDALIASES" >> /tmp/petget-proc/petget_installed_patterns

#w480 pkg_chooser has created this, pkg names that need to be ignored (for whatever reason)...
cat /tmp/petget-proc/petget_pkg_name_ignore_patterns >> /tmp/petget-proc/petget_installed_patterns
...
BADKERNPTNS="`grep -o '\-k2\.6\.[^-|a-zA-Z]*' /tmp/petget-proc/petget_fltrd_repo_${PKG_FIRST_CHAR}_${fltrCATEGORY}_${xDEFGUIFILTER}_Packages-${fltrREPO_TRIAD} | cut -f 1 -d '|' | grep -v "$GOODKERNPTN" | sed -e 's%$%-%' -e 's%\.%\\\.%g' -e 's%\-%\\\-%g'`" #ex: \-k2\.6\.32\.28\-
[ "$BADKERNPTNS" ] && echo "$BADKERNPTNS" >> /tmp/petget-proc/petget_installed_patterns
...
if [ "$xDBC" != "puppy" ];then #not PET pkgs.
 ...
 do
  ...
  [ "fONEPTBCK" != "" ] && echo '+'"$ONEPTBCK"'[,|]' >> /tmp/petget-proc/petget_installed_patterns
 done
fi

#clean it up...
grep -v '^$' /tmp/petget-proc/petget_installed_patterns > /tmp/petget-proc/petget_installed_patterns-tmp
